...
class Login extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('aulakoronka/Login_model');
    }
    public function index(){
        $this->load->view('aulakoronka/Login_view');
        if($this->session->userdata('rememberme')=="1")
        {
            redirect(site_url('aulakoronka/Home'));
        }
    }
    ...
    public function submit(){
        if($this->input->post('Login')){
            ...
            if($cek){
                ...
                $this->load->view('aulakoronka/Login_view',$this->data);
            }
        }else{
            redirect(site_url('aulakoronka/Login'));
        }
    }
} 
